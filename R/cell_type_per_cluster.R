...
cell_type_per_cluster=function(seurat_object, output="count"){
  ...
  for (i in 1:length(clusters)){
    ...
    if (output=="count"){
    df=data.frame(row.names = subset_celltype_names, subset_celltype_counts)
    celltype_cluster_list[[i]]=df
    } else if(output=="percent") {
    df=data.frame(row.names = subset_celltype_names, subset_celltype_counts/sum(subset_celltype_counts))
    celltype_cluster_list[[i]]=df
    }
  }
  return(celltype_cluster_list)
}
