...
make_star_matrix=function(mtx_stardir){
  barcodes=read_tsv(paste0(mtx_stardir,"/","barcodes.tsv"), col_names = F)
  barcodes=as.character(unlist(barcodes))
  features=read_tsv(paste0(mtx_stardir,"/","features.tsv"), col_names = F)
  ensg=as.character(unlist(features[,1]))
  genename=as.character(unlist(features[,2]))
  mat=readMM(paste0(mtx_stardir,"/","matrix.mtx"))
